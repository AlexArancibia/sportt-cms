...
export default function CardSectionsPage() {
  ...
  const { cardSections, fetchCardSectionsByStore, currentStore, stores, fetchStores } = useMainStore()
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        // Si hay una tienda seleccionada, cargar las secciones de tarjetas
        if (currentStore) {
          await fetchCardSectionsByStore(currentStore)
        }
      } catch (err) {
        console.error("Error al cargar datos:", err)
        setError("Error al cargar las secciones de tarjetas. Por favor, inténtelo de nuevo.")
      } finally {
        setIsLoading(false)
      }
    }
    ...
    loadData()
  }, [currentStore, fetchCardSectionsByStore, fetchStores, stores.length])
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      setError("Error al refrescar las secciones de tarjetas. Por favor, inténtelo de nuevo.")
    } finally {
    ...
    <div className="container mx-auto py-6 px-4">
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-12">
          ...
          <p className="text-muted-foreground">Cargando secciones de tarjetas...</p>
        </div>
      ) : error ? (
        ...
        <Alert className="mb-6">
          ...
          <AlertDescription>Seleccione una tienda para ver las secciones de tarjetas.</AlertDescription>
        </Alert>
      ) : (
