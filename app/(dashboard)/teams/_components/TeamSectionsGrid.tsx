...
import { Edit, Eye, Trash2, Users, Palette, LayoutGrid } from "lucide-react"
import { useToast } from "@/hooks/use-toast"
import { formatDate } from "@/lib/utils"
import { motion } from "framer-motion"
import { TeamSection } from "@/types/team"

interface TeamSectionsGridProps {
  teamSections: TeamSection[]
}

export function TeamSectionsGrid({ teamSections }: TeamSectionsGridProps) {
    ...
    <>
      <div className="p-4">
        {teamSections.length === 0 ? (
          <div className="text-center py-12 text-muted-foreground">No hay secciones de equipo disponibles.</div>
        ) : (
                    ...
                    <div className="space-y-2">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        <div className="flex items-center text-xs bg-muted text-muted-foreground rounded-md px-2 py-1">
                          ...
                          <span>{section.members?.length || 0} miembros</span>
                        </div>
                        {section.layout && (
                          <div className="flex items-center text-xs bg-muted text-muted-foreground rounded-md px-2 py-1">
                  ...
                  <CardFooter className="flex justify-between pt-2">
                    ...
                    <div className="flex gap-1">
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8"
                        onClick={() => router.push(`/team-sections/${section.id}`)}
                      >
                      ...
                      <Button
                        ...
                        size="icon"
                        className="h-8 w-8"
                        onClick={() => router.push(`/team-sections/${section.id}/edit`)}
                      >
